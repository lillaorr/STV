stv <- function(x, seats = 1, file = "", surplusMethod = "Cambridge", quotaMethod = "Droop") {
  ...
  if(!surplusMethod == "Cambridge") stop("Please set surplusMethod = Cambridge") # Currently support
  if(!quotaMethod == "Droop") stop("Please set quotaMethod = Droop")
  ...
  included.ballots <- rep(TRUE, nrow(x))      # Ballots excluded by surplus redistribution (Invert this to included.ballots)
  # names(excluded.ballots) <- rownames(x)       # Will use names during surplus allocation
  unfilled <- seats                            # Vacant seats at a given round
  Nround <- 0                                  # Current number of round

  full.results <- data.frame(matrix(NA, ncol = 9 + ncol(x), nrow = 0)) # rename as res
  names(full.results) <- c("ballots", "seats.to.fill", "quota", "max.vote.count", "min.vote.count",
                           "elim.cand", "tied.for.elim", "elect.cand", "surplus", names(x))

  # Iteratively elect/eliminate candidates till all seats are filled:
  while (unfilled > 0) {
    Nround <- Nround + 1
    full.results[Nround, ] <- rep(NA, ncol(full.results))
    full.results$seats.to.fill[Nround] <- unfilled
    ...
    if (unfilled == length(curr.candidates)) {
      ...
      full.results$elect.cand[Nround] <- paste(curr.candidates, collapse = "; ")
      full.results$seats.to.fill[Nround] <- 0
      full.results[Nround, elect] <- "Elected"
      full.results[Nround, elim] <- "Eliminated"
      full.results$quota[Nround] <- "Won by elim"
    ...
    full.results$ballots[Nround] <- ballot.size
    ...
    full.results$quota[Nround] <- quota
    ...
    full.results[Nround, names(vote.counts)] <- vote.counts
    full.results$max.vote.count[Nround] <- max(vote.counts)
    full.results$min.vote.count[Nround] <- min(vote.counts)
    full.results[Nround, elect] <- "Elected"
    full.results[Nround, elim] <- "Eliminated"
